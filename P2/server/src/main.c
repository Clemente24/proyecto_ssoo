...
void seleccionar_clase(char *client_message, int indice)
{
  ...
  lista_jugadores[indice].espina_venenosa = 0;
...
void ejecutar_poder(Jugador jugador, char *client_message)
{
  ...
  {
    if (seleccion == 1)
      cazador_estocada(monstruo);
    else if (seleccion == 2)
      cazador_corte_cruzado(monstruo);
    else if (seleccion == 3)
      cazador_distraer();
  }
  ...
  {
    if (seleccion == 1)
      medico_curar(jugador);
    else if (seleccion == 2)
      medico_destello_regenerador(jugador, monstruo);
    else if (seleccion == 3)
      medico_descarga_vital(jugador, monstruo);
  }
  else
  {
    if (seleccion == 1)
...
void *thread_cliente(void *arg)
{
  ...
  while (1)
  {
    ...
    { //recibe accion envia un turno nuevo a siguiente jugador
      ...
      if (validar_respuesta(4, opciones, client_message))
      {
        /* Añadir funcion de Venenos/Sangrados
        - controlar sangrado de monstruo x estocada
        - controlar envenenamiento jugadores
        */
